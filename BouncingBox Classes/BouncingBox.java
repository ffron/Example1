...
public class BouncingBox { 
	...
	public void draw(Graphics surface){
		...
		// store current stroke thickness
		Stroke strokeThickness = ((Graphics2D) surface).getStroke();
		// Make a thick stroke for the outside of the box
		((Graphics2D) surface).setStroke(new BasicStroke(3.0f));
		// Draw Rectangle
		surface.drawRect(x - SIZE/2, y - SIZE/2, SIZE, SIZE);
		// return stroke thickness to original stroke size
		((Graphics2D) surface).setStroke(strokeThickness);
